...
class Image {
  constructor(options) {
    ...
    const { url, filename, caption, dom, active, editable } = mergeSettings(
      options,
      defaults
    );
    ...
    this._captionSelector =
      this._dom && this._dom.querySelector("[contenteditable]");
    this._active = active;
    this._status = false;
  ...
  removeListeners() {
    if (this._dom) {
      this._dom.removeEventListener("click", this.handleClick);
  ...
  nextImage() {
    document.dispatchEvent(new Event(EVENT_NEXT_IMAGE));
  }
  ...
  dispatchStatusUpdate(event) {
    ...
    document.dispatchEvent(
      new CustomEvent(EVENT_IMAGE_UPDATE, {
        detail: {
    ...
    if (src) {
      return htmlToElement(`<div class="Image">
        <div class="Image__container">
          <img class="lazy miniarch" src="./assets/css/loading.gif" data-src="${removeHtml(
            src
          )}" data-filename="${removeHtml(
        filename
      )}" title="${filename} preview" />
        </div>
        <div class="Image__caption"><span contenteditable="true">${removeHtml(
          caption
        )}</span></div>
        </div>`);
    }
  ...
  get dom() {
    return this._dom
      ? this._dom
      : this.generateDom(this._src, this._filename, this._caption);
  }
